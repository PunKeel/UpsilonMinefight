...
public class Arenas {
    ...
    public void init() {
        ...
        temp_sign = Bukkit.getWorld(Main.WORLDNAME).getBlockAt(7, 53, 275);
        if (temp_sign.getType().equals(Material.WALL_SIGN))
            sign_nether = (Sign) temp_sign.getState();


        temp_sign = Bukkit.getWorld(Main.WORLDNAME).getBlockAt(7, 53, 261);
        if (temp_sign.getType().equals(Material.WALL_SIGN))
            sign_normal = (Sign) temp_sign.getState();
        if (sign_normal != null) {
            sign_normal.setLine(0, "Il y a");
            sign_normal.setLine(1, "0 joueur");
        ...
        if (sign_nether != null) {
            ...
            sign_nether.setLine(1, "0 joueur");
    ...
    public Integer getIn_normal() {
        if (in_normal == null) return 0;
        return in_normal;
    }

    public Integer getIn_nether() {
        if (in_nether == null) return 0;
        return in_nether;
    }
}
